...
export interface Producto {
  ...
  estado: string;        // <- lo dejamos como string para evitar problemas de tipos
  es_tuyo?: boolean;
}
...
export class ProductosService {
  ...
  listar(opts?: {
    ...
    if (opts?.id_categoria) params = params.set('id_categoria', String(opts.id_categoria));
    if (opts?.ubicacion) params = params.set('ubicacion', opts.ubicacion);
  ...
  // =========================
  cambiarEstado(id_producto: number, estado: string): Observable<Producto> {
  ...
  actualizarEstado(id_producto: number, estado: string): Observable<Producto> {
